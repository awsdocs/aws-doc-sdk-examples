...
for lang in "${languages[@]}"; do
  mkdir -p "$1/content/$lang"
  pushd "$1/content/$lang"
...
EOF
  ...
  # Add 01_README.md file
  cat << EOF > 01_README.md
---
parent: always
prompt: |
...
EOF
  ...
  # Create 02_SPECIFICATION.md
  cat << EOF > 02_SPECIFICATION.md
---
parent: always
prompt: |
...
---
prompt: |
    Persona: You are a guru-level solutions engineer with expertise in $lang and AWS $1.
    Task: Sketch out a workflow using the AWS SDK for $lang to interface with the $1.
    Requirements:
      ...
      Code and Configuration Files:
         - List all necessary files, including source code in $lang, and CDK or CFN setup files in TypeScript.
...
prompt: |
    Persona: You are a guru-level solutions engineer with expertise in $lang and AWS $1.
    Task: Write code for a workflow using the AWS SDK for $lang to interface with the $1.
    Output:
...
mv .quip.txt > $1/.aillyrc

root=$PWD

for lang in "${languages[@]}"; do
  cd "$root"/"$1"/content/"$lang"
  ./run.sh
done
