...
class CloudWatchQuery:
    ...
    def __init__(self, log_group_names, date_range, limit=10000):
        self.logs_client = boto3.client('logs', config=Config(retries={'max_attempts': 10}))
    ...
    def _large_query(self, date_range):
        ...
        print(f"Query date range: {date_range[0].isoformat()} to {date_range[1].isoformat()}. Found {len(logs)} logs.")
