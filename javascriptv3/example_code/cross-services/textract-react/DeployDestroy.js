...
/**
 * This script deploys and destroys additional resources for the example after the
 ...
 *
 * ** Deploy **
 * Deploy the additional demo resources by running the following at a command prompt:
 *
 ...
 *
 * ** Destroy **
 * Destroy the additional demo resources by running the following at a command prompt:
 *
...
import {CognitoIdentityClient, DeleteIdentitiesCommand, ListIdentitiesCommand} from "@aws-sdk/client-cognito-identity";
import {CloudFormationClient, DescribeStacksCommand} from "@aws-sdk/client-cloudformation";

const ConfigError = 'Demo resources not initialized. You must deploy AWS resources ' +
...
if (mode === 'deploy') {
  ...
  (async () => {
    try {
      const {Stacks} = await cform.send(new DescribeStacksCommand({
        StackName: stackName
      }));
      let configOutputs = (
        `  StackName: '${stackName}',\n` +
      ...
      console.log(
        `Deploying demo image ${DefaultImageName} to ` +
      ...
      console.log("Demo elements successfully deployed!");
    } catch (error) {
  ...
  (async () => {
    try {
      const {Users} = await cogProvider.send(new ListUsersCommand({
        UserPoolId: Config.CognitoUserPoolId
      }));
      Users.forEach(user => {
        console.log(`Deleting user ${user.Username}`);
        cogProvider.send(new AdminDeleteUserCommand({
          UserPoolId: Config.CognitoUserPoolId, Username: user.Username
        }));
      });
    } catch (error) {
      console.log(error.message);
  ...
  (async () => {
    try {
      const {Identities} = await cogIdentity.send(new ListIdentitiesCommand({
        IdentityPoolId: Config.CognitoIdentityPoolId,
        MaxResults: 10
      }));
      const idList = Identities.map(id => id.IdentityId);
      console.log(`Removing identities ${idList} from identity` +
        `pool ${Config.CognitoIdentityPoolId}.`);
      await cogIdentity.send(new DeleteIdentitiesCommand({
    ...
    console.log("Demo elements removed. Now run `aws cloudformation delete-stack " +
      "stack-name <stack-name>` to delete resources deployed for the demo.");
  })();
} else {
  console.log("Run with `deploy <stack-name>` to deploy demo resources.\n" +
    "Run with `destroy` to remove them.");
}