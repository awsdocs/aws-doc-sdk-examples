...
import {
  TextractClient, DetectDocumentTextCommand, AnalyzeDocumentCommand, FeatureType,
...
describe("loadImage", () => {
  test("reads from stream, returns base 64 data", async () => {
    ...
    s3.send = jest.fn((command) => {
      ...
      return {
        Body: {
          getReader: () => ({
            read: jest.fn()
              .mockReturnValueOnce({done: false, value: {reduce: () => imgValue}})
    ...
    expect(data).toEqual({bucketName: bucket, objectKey: key, base64Data: btoa(imgValue)});
  ...
  test("fills modelError when error raised", () => {
    ...
    const s3 = new S3Client({});
    s3.send = jest.fn((command) => { throw new Error(testError); });
...
describe("extractDocument", () => {
  test("synchronous text type uses detect command, returns expected document", async () => {
    ...
    const imageData = {bucketName: "test-bucket", objectKey: TestExtractDocument.Name};
  ...
  test("modelError contains error message when error thrown", async () => {
    ...
    const textract = new TextractClient({});
    textract.send = jest.fn((command) => { throw new Error(testError); });
    const tm = new TextractModel({textract: textract});
  ...
  test("asynchronous text type calls detect command, waits for sqs, returns " +
    ...
    sqs.send = jest.fn()
      .mockImplementationOnce((command) => {
        ...
        expect(command.input).toEqual({QueueUrl: queueUrl, MaxNumberOfMessages: 1});
        return {
          Messages: [{
            ReceiptHandle: 'receipt-handle',
            Body: JSON.stringify({Message: JSON.stringify({Status: JobStatus.SUCCEEDED})})
          }]
        };
      })
      .mockImplementationOnce((command) => {
    ...
    const tm = new TextractModel({
      textract: textract, sqs: sqs, snsTopicArn: snsTopicArn, roleArn: roleArn,
      queueUrl: queueUrl});
    tm.imageData = imageData;
    tm.inform = jest.fn();
    await tm.extractDocument(syncType, extractType)
  ...
  test('setTimeout called when no messages yet', async () => {
    ...
    const textract = new TextractClient({});
    textract.send = jest.fn()
      .mockImplementationOnce(() => {return {JobId: jobId};})
      .mockImplementationOnce(() => {return {Blocks: []};});
    const sqs = new SQSClient({});
    sqs.send = jest.fn()
