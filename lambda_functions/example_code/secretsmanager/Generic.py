<<<<<<< HEAD:lambda_functions/secretsmanager/Generic.py
...
def finish_secret(service_client, arn, token):
    ...
    service_client.update_secret_version_stage(SecretId=arn, VersionStage="AWSCURRENT", MoveToVersionId=token, RemoveFromVersionId=current_version)
    logger.info("finishSecret: Successfully set AWSCURRENT stage to version %s for secret %s." % (version, arn))
>>>>>>> ea7e8cc8... moved all examples into example_code child folder per standard:lambda_functions/example_code/secretsmanager/Generic.py
