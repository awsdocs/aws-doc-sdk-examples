<<<<<<< HEAD:lambda_functions/secretsmanager/RDSPostgreSQL-Multiuser.py
...
def get_alt_username(current_username):
    ...
    if current_username.endswith(clone_suffix):
        ...
        if len(new_username) > 63:
            raise ValueError("Unable to clone user, username length with _clone appended would exceed 63 characters")
        return new_username
>>>>>>> ea7e8cc8... moved all examples into example_code child folder per standard:lambda_functions/example_code/secretsmanager/RDSPostgreSQL-Multiuser.py
