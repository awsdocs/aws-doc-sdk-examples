...
{
    // snippet-start:[Rekognition.dotnetv4.FaceRekognitionExample]
    using System;
    ...
    public class FaceRekognition
    {
        ...
        public static async Task IdentifyFaces(AmazonRekognitionClient client, string filename)
        {
            ...
            if (response.FaceDetails.Count > 0)
            {
                ...
                {
                    foreach (var bb in from fd in response.FaceDetails// Get the bounding box.
                                       let bb = fd.BoundingBox
                                       select bb)
                    {
                        ...
                        graphics.DrawRectangle(
                                                    pen,
                                                    x: (float)(facesHighlighted.Width * bb.Left),
                                                    y: (float)(facesHighlighted.Height * bb.Top),
                                                    width: (float)(facesHighlighted.Width * bb.Width),
                                                    height: (float)(facesHighlighted.Height * bb.Height));
                    }
                }
                ...
                using (var graphics = Graphics.FromImage(facesHighlighted))
                {
                    foreach (var fd in response.CelebrityFaces)
                    {
                        ...
                        graphics.DrawString(
                            ...
                            brush: Brushes.White,
                            x: (float)(facesHighlighted.Width * bb.Left),
                            y: (float)(facesHighlighted.Height * bb.Top) + (float)(facesHighlighted.Height * bb.Height));
                    }
                }
    ...
    // snippet-end:[Rekognition.dotnetv4.FaceRekognitionExample]
}