...
# List all instance snapshots for an Amazon Relational Database Service (Amazon RDS) cluster
#
# @param rds_resource [Aws::RDS::Resource]: The resource containing SDK logic
# @return instance_snapshots [Array]: All instance snapshots
def list_instance_snapshots(rds_resource)
  instance_snapshots = []
  rds_resource.db_snapshots.each do |s|
    instance_snapshots.append({"id": s.snapshot_id, "status": s.status})
  end
  instance_snapshots
