...
# Enables working with Amazon DynamoDB attribute values using hashes, arrays, sets, integers, floats, Booleans, and nil.
# Disabling this option requires that all attribute values have their types specified, for example `{ s: 'abc' }`
# instead of `'abc'`.
...
# Initialize all arguments passed to the function into an array called args
module Aws
  module DynamoDB
...
# if the requested item is not present in the table, an appropriate error message is returned
  if req.item == nil
    ...
    puts "Could not find the Director, Time in Theatre, or Award Winner Status': #{e} (#{e.class})"
    exit 0
...
# Rescuing an error returned from the service (not one generated by the client) with an accompanying error message
    if results.SERVER_ERROR == result
      ...
      puts "Unable to find the Director, Time in Theatre, or Award Winner Status': #{e} (#{e.class})"
      exit 0
    end
  end
end
