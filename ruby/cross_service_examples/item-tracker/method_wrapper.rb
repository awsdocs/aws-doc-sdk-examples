...
class MethodWrapper
  ...
  def initialize(config)
    @db_wrapper = DBWrapper.new(config, Aws::RDS::Client.new)
  ...
  def get(item_id = nil, include_archived = nil)
    status = 200
    response = @db_wrapper.get_work_items(item_id, include_archived)
  ...
  # @param item_json: The request body data, validated and transformed by the work item schema.
  # @return: The generated ID of the newly added work item, and an HTTP result code.
  def post(item_json)
    status = 200
    @logger.info("Attempting to add item:\n #{item_json}")
    response = @db_wrapper.add_work_item(item_json)
  ...
  def archive(item_id)
    response = @db_wrapper.archive_work_item(item_id)
    status = 200
