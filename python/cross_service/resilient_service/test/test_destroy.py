...
class MockManager:
    ...
    def setup_stubs(self, error, stop_on):
        with self.stub_runner(error, stop_on) as runner:
            runner.add(
                self.scenario_data.elb.stubber.stub_describe_load_balancers, [self.scenario_data.lb_name], arns=[self.scenario_data.lb_arn])
            runner.add(self.scenario_data.elb.stubber.stub_delete_load_balancer, self.scenario_data.lb_arn)
            runner.add(
                self.scenario_data.elb.stubber.stub_describe_load_balancers, [self.scenario_data.lb_name], error_code='LoadBalancerNotFound')
            runner.add(
                self.scenario_data.elb.stubber.stub_describe_target_groups, [self.scenario_data.tg_name], [self.scenario_data.tg_arn])
            runner.add(self.scenario_data.elb.stubber.stub_delete_target_group, self.scenario_data.tg_arn)
            runner.add(
                self.scenario_data.auto_scaling.stubber.stub_describe_auto_scaling_groups, [self.scenario_data.asg_name], [self.scenario_data.asg_group])
            ...
            runner.add(
                ...
                self.scenario_data.instance['InstanceId'], True, None)
            ...
            runner.add(
                ...
                self.scenario_data.profile_name, self.scenario_data.role_name)
            runner.add(self.scenario_data.iam.stubber.stub_delete_instance_profile, self.scenario_data.profile_name)
            runner.add(
                self.scenario_data.iam.stubber.stub_list_attached_role_policies, self.scenario_data.role_name,
                {'1': self.scenario_data.policy_arn})
            runner.add(
                self.scenario_data.iam.stubber.stub_detach_role_policy, self.scenario_data.role_name, self.scenario_data.policy_arn)
            ...
            runner.add(
                self.scenario_data.iam.stubber.stub_remove_role_from_instance_profile,
                self.scenario_data.bad_prof_name, self.scenario_data.bad_role_name)
            runner.add(self.scenario_data.iam.stubber.stub_delete_instance_profile, self.scenario_data.bad_prof_name)
            runner.add(
                self.scenario_data.iam.stubber.stub_list_attached_role_policies, self.scenario_data.bad_role_name,
                {'1': self.scenario_data.bad_policy_arn})
            runner.add(
                self.scenario_data.iam.stubber.stub_detach_role_policy, self.scenario_data.bad_role_name, self.scenario_data.bad_policy_arn)
...
def test_destroy_error(
        mock_mgr, caplog, error, stub_name, stop_on_index):
