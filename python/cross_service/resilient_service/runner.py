...
class Runner:
    def __init__(self, resource_path, recommendation, autoscaler, loadbalancer, param_helper):
    ...
    def deploy(self):
        ...
        print("\t* A DynamoDB table that the web service depends on to provide book, movie, and song recommendations.")
        print("\t* An EC2 launch template that defines EC2 instances that each contain a Python web server.")
        print("\t* An EC2 Auto Scaling group that manages EC2 instances across several Availability Zones.")
        print("\t* An Elastic Load Balancing (ELB) load balancer that targets the Auto Scaling group to distribute requests.")
        print('-'*88)
        q.ask("Press Enter when you're ready to start deploying resources.")

        print(f"Creating and populating a DynamoDB table named '{self.recommendation.table_name}'.")
        self.recommendation.create()
        self.recommendation.populate(recommendations_path)
        print('-'*88)

        print(f"Creating an EC2 launch template that runs '{startup_script}' when an instance starts.\n"
        ...
        print(f"Creating an EC2 Auto Scaling group that maintains three EC2 instances, each in a different\n"
              f"Availability Zone.")
        zones = self.autoscaler.create_group(3)
        print('-'*88)
        print("At this point, you have EC2 instances created. Once each instance starts, it listens for\n"
        ...
        target_group = self.loadbalancer.create_target_group(self.protocol, self.port, vpc['VpcId'])
        self.loadbalancer.create_load_balancer([subnet['SubnetId'] for subnet in subnets], target_group)
        ...
        if lb_success:
            ...
            print("Couldn't get a successful response from the load balancer endpoint. Troubleshoot by\n"
        ...
        while choice != 2:
            ...
            if choice == 0:
                ...
                for target in health:
                    ...
                    print(f"\tTarget {target['Target']['Id']} on port {target['Target']['Port']} is {state}")
                    if state != 'healthy':
                        print(f"\t\t{target['TargetHealth']['Reason']}: {target['TargetHealth']['Description']}\n")
    ...
    def demo(self):
        ...
        print("At the start, the load balancer endpoint returns recommendations and reports that all targets are healthy.")
        self.demo_choices()

        print(f"The web service running on the EC2 instances gets recommendations by querying a DynamoDB table.\n"
        ...
        self.autoscaler.replace_instance_profile(
            bad_instance_id, self.autoscaler.bad_creds_profile_name, instance_profile['AssociationId'])
        print("Now, sending a GET request to the load balancer endpoint returns either a recommendation or a static response,\n"
    ...
    def destroy(self):
        ...
        if q.ask("Do you want to clean up all demo resources? (y/n) ", q.is_yesno):
            ...
            print("Okay, we'll leave the resources intact.\n"
...
def main():
    ...
    parser.add_argument(
        '--resource_path', default='../../../workflows/resilient_service/resources',
        help="The path to resource files used by this example, such as IAM policies and\n"
             "instance scripts.")
    args = parser.parse_args()

    print('-'*88)
    print("Welcome to the demonstration of How to Build and Manage a Resilient Service!")
    print('-'*88)
