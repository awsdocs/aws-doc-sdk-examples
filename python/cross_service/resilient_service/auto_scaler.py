...
class AutoScaler:
    ...
    """
    def __init__(
            self, resource_prefix, inst_type, ami_param, autoscaling_client,
    ...
    def from_client(cls, resource_prefix):
        ...
        return cls(
            resource_prefix, 't3.micro', '/aws/service/ami-amazon-linux-latest/amzn2-ami-hvm-x86_64-gp2',
            as_client, ec2_client, ssm_client, iam_client)
        ...
        try:
            ...
            for aws_policy in aws_managed_policies:
                self.iam_client.attach_role_policy(
                    RoleName=role_name, PolicyArn=f'arn:aws:iam::aws:policy/{aws_policy}')
    ...
    def replace_instance_profile(
        ...
        try:
            ...
            log.info("Replaced instance profile for association %s with profile %s.",
                     profile_association_id, new_instance_profile_name)
            time.sleep(5)
            inst_ready = False
            tries = 0
    ...
    def get_instances(self):
        ...
        try:
            ...
            return ec2_response['Reservations'][0]['Instances']
    ...
    def attach_load_balancer_target_group(self, lb_target_group):
        ...
        try:
            self.autoscaling_client.attach_load_balancer_target_groups(
                AutoScalingGroupName=self.group_name, TargetGroupARNs=[lb_target_group['TargetGroupArn']])
            log.info(
                "Attached load balancer target group %s to auto scaling group %s.",
                lb_target_group['TargetGroupName'], self.group_name)
        except ClientError as err:
            raise AutoScalerError(
    ...
    def _try_delete_group(self):
        ...
        while not stopped:
            try:
                ...
                if (err.response['Error']['Code'] == 'ResourceInUse' or
                        err.response['Error']['Code'] == 'ScalingActivityInProgress'):
