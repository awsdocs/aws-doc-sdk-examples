...
class MockManager:
    def __init__(self, stub_runner, scenario_data, input_mocker):
        ...
        self.communications = [{
            'body': f'Communication-{i}', 'caseId': f'test-case-{i}', 'timeCreated': f'test-time{i}',
            'attachmentSet': []} for i in
            range(1, 4)]
...
@pytest.mark.parametrize('error, stop_on_index', [
    ('TESTERROR-stub_describe_communications', 0)
])
def test_list_communications_error(
