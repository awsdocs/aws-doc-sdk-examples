...
def test_ml_connector():
    with patch("builtins.open", mock_open()):
        with patch.object(
            gg_ml_mock.client(), "invoke_inference_service"
        ) as invoke_mock:
            mock_body = MagicMock()
...
def test_local_resource_access(monkeypatch):
    ...
    gg_mock.client().publish.assert_has_calls(
        [call(topic="LRA/test", payload=ANY), call(topic="LRA/test", payload=ANY)]
    )


@pytest.mark.parametrize(
    "module_name,check_publish,use_version",
    [
