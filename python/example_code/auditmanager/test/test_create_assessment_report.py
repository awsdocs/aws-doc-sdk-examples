...
@pytest.mark.parametrize(
    "assessment_uuid, evidence_date, tokens, folders, stop_on_action, error_code",
    [
        (
            uuid.UUID("f66e7fc4-baf1-4661-85db-f6ff6ee76630"),
            None,
        ...
        (
            ...
            [None, None],
            [],
            "stub_get_evidence_folders_by_assessment",
            "TestException",
        ),
        (
            uuid.UUID("f66e7fc4-baf1-4661-85db-f6ff6ee76630"),
            ...
            [
                {
                    "id": f'id-{"1" * 36}',
                    "name": "2021-01-01",
                    "assessmentReportSelectionCount": 2,
                    "totalEvidence": 1,
                    "controlId": f'ctl-{"1" * 36}',
                }
            ],
...
def test_clear_staging(
    ...
    with stub_runner(error_code, stop_on_action) as runner:
        ...
        if len(folders) > 0:
            if (
                folders[0]["assessmentReportSelectionCount"]
                == folders[0]["totalEvidence"]
            ):
                runner.add(
...
def test_get_report(make_stubber, stub_runner, monkeypatch, error_code, stop_on_action):
    ...
    with stub_runner(error_code, stop_on_action) as runner:
        runner.add(
            auditmanager_stubber.stub_create_assessment_report,
            "ReportViaScript",
            "testing",
            str(assessment_uuid),
            report_id,
