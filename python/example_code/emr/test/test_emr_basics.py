...
def test_run_job_flow(make_stubber, make_unique_name, error_code):
    ...
    security_groups = {
        "manager": MagicMock(id="sg-1234"),
        "worker": MagicMock(id="sg-5678"),
    }
...
def test_describe_cluster(make_stubber, error_code):
    ...
    emr_stubber.stub_describe_cluster(
        cluster_id, cluster, error_code=error_code)
...
def test_list_steps(make_stubber, error_code):
    ...
    cluster_id = "test-id"
    steps = [
        {"Id": f"id-{index}", "Name": f"step-{index}", "Status": {"State": "COMPLETED"}}
