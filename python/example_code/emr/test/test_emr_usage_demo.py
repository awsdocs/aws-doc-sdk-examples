...
def test_create_roles(
    ...
    if error_code_jf is None and error_code_svc is None:
        ...
        assert (
            exc_info.value.response["Error"]["Code"] == error_code_jf
            or exc_info.value.response["Error"]["Code"] == error_code_svc
        )
...
def test_delete_roles(
    ...
    with stub_runner(error_code, stop_on_method) as runner:
        for role in roles:
            ...
            runner.add(
                iam_stubber.stub_remove_role_from_instance_profile,
                inst_profile,
                role.name,
            )
