...
class AcmCertificate:
    ...
    def describe(self, certificate_arn):
        ...
        try:
            ...
            logger.exception(
                "Couldn't get data for certificate %s.",
                certificate_arn)
            raise
        else:
            return certificate
    ...
    def get(self, certificate_arn):
        ...
        """
        try:
            response = self.acm_client.get_certificate(
                CertificateArn=certificate_arn)
    ...
    def add_tags(self, certificate_arn, tags):
        ...
        try:
            ...
            logger.info(
                "Added %s tags to certificate %s.",
                len(tags),
                certificate_arn)
        except ClientError:
            logger.exception(
                "Couldn't add tags to certificate %s.",
                certificate_arn)
            raise
    ...
    def list_tags(self, certificate_arn):
        ...
        try:
            ...
            logger.info(
                "Got %s tags for certificates %s.",
                len(tags),
                certificate_arn)
        except ClientError:
            logger.exception(
                "Couldn't get tags for certificate %s.",
                certificate_arn)
            raise
        else:
            return tags
    ...
    def remove_tags(self, certificate_arn, tags):
        ...
        try:
            ...
            logger.info(
                "Removed %s tags from certificate %s.",
                len(tags),
                certificate_arn)
        except ClientError:
            logger.exception(
    ...
    def request_validation(
        ...
        try:
            ...
            logger.exception(
                "Request for %s validation of domain %s failed.",
                method,
                domain)
            raise
        else:
...
def usage_demo():
    ...
    certificate_arn = acm_certificate.request_validation(
        domain, sub_domains, "DNS")
    ...
    if cert_file_name:
        ...
        if pk_file_name:
            ...
            import_cert_arn = acm_certificate.import_certificate(
                import_cert, import_pk)
    ...
    if import_cert_arn is not None:
        print(
            f"Getting certificate for imported certificate {import_cert_arn}")
    ...
    acm_certificate.add_tags(
        certificate_arn, {
            "purpose": "acm demo", "color": "green"})
