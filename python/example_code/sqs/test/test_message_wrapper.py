...
@pytest.mark.parametrize(
    "body_template,attributes,count",
    [
...
def test_send_messages(make_stubber, make_queue, body_template, attributes, count):
    ...
    messages = [
        {"body": body_template.format(ind), "attributes": attributes}
...
def test_delete_message_not_exist(make_stubber, make_queue):
    ...
    sqs_stubber.stub_delete_message(
        queue.url, message, error_code="ReceiptHandleIsInvalid"
    )
...
def test_delete_messages_not_exist(make_stubber, make_queue):
    ...
    assert all(
        [failed["Code"] == "ReceiptHandleIsInvalid" for failed in response["Failed"]]
    )
