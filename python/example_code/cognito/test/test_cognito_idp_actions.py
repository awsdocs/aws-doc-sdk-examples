...
def test_sign_up_user(make_stubber, client_secret, error_code):
    ...
    cognito_idp_stubber.stub_sign_up(
        ...
        email,
        confirmed,
        None if client_secret is None else ANY,
...
@pytest.mark.parametrize(
    "client_secret,error_code",
    [(None, None), ("test-secret", None), (None, "TestException")],
)
def test_resend_confirmation(make_stubber, client_secret, error_code):
    ...
    client_id = "test-client-id"
    wrapper = CognitoIdentityProviderWrapper(
        cognito_idp_client, "", client_id, client_secret
    )
    user_name = "test-user_name"
...
@pytest.mark.parametrize(
    "error_code, stop_on_method, client_secret",
    [
        (None, None, None),
...
def test_respond_to_mfa_challenge(make_stubber, error_code, client_secret):
    ...
    if error_code is None:
        got_access_token = wrapper.respond_to_mfa_challenge(
            user_name, session, mfa_code
        )
...
def test_sign_in_with_tracked_device(
