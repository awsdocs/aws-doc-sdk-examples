...
class MockManager:
    def __init__(self, stub_runner, cluster_data, input_mocker):
        ...
        self.group = {
            ...
            "DBClusterParameterGroupName": "test-group",
        }
        self.engine_versions = [
            {"EngineVersion": f"test-version-{i}"} for i in range(1, 4)
        ]
    ...
    def setup_stubs(self, error, stop_on, stubber):
        with self.stub_runner(error, stop_on) as runner:
            runner.add(
                stubber.stub_describe_db_clusters,
                self.cluster_name,
            ...
            runner.add(
                ...
                engine_version=self.engine_versions[self.engine_choice - 1][
                    "EngineVersion"
                ],
            )
...
def test_create_cluster_exists(mock_mgr, capsys):
    ...
    assert (
        f"'DBClusterIdentifier': '{mock_mgr.scenario_out['DBClusterIdentifier']}'"
        in capt.out
    )
