...
class MockManager:
    ...
    def setup_stubs(self, error, stop_on, stubber):
        with self.stub_runner(error, stop_on) as runner:
            runner.add(
                stubber.stub_describe_db_cluster_parameter_groups,
                self.group_name,
                [],
            ...
            runner.add(
                stubber.stub_create_db_cluster_parameter_group,
                self.group_name,
                "test-group-1",
            )
            runner.add(
...
def test_create_parameter_group_exists(mock_mgr, capsys):
    ...
    got_group = mock_mgr.cluster_data.scenario.create_parameter_group(
    ...
    assert (
        f"'DBClusterParameterGroupName': '{mock_mgr.scenario_out['DBClusterParameterGroupName']}'"
        in capt.out
    )
