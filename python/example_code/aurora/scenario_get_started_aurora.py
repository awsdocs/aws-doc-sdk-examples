...
class AuroraClusterScenario:
    ...
    def create_parameter_group(self, db_engine, parameter_group_name):
        ...
        print(
            f"Parameter group {parameter_group['DBClusterParameterGroupName']}:")
        pp(parameter_group)
        print("-" * 88)
    ...
    def set_user_parameters(self, parameter_group_name):
        ...
        self.aurora_wrapper.update_parameters(
    ...
    def create_instance(self, cluster):
        ...
        if db_inst is None:
            ...
            print(
                "Creating a database instance. This typically takes several minutes."
            )
    ...
    def create_snapshot(self, cluster_name):
        ...
        if q.ask(
            ...
            while snapshot.get("Status") != "available":
                wait(30)
                snapshot = self.aurora_wrapper.get_cluster_snapshot(
                    snapshot_id)
            pp(snapshot)
            print("-" * 88)
    ...
    def cleanup(self, db_inst, cluster, parameter_group):
        ...
        if q.ask(
            ...
            print(
                f"Deleting database instance {db_inst['DBInstanceIdentifier']}.")
            self.aurora_wrapper.delete_db_instance(
    ...
    def run_scenario(
        ...
        parameter_group = self.create_parameter_group(
            db_engine, parameter_group_name)
        self.set_user_parameters(parameter_group_name)
        cluster = self.create_cluster(
            cluster_name, db_engine, db_name, parameter_group)
        wait(5)
