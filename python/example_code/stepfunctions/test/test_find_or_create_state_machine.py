    ...
    def setup_stubs(self, error, stop_on, stubber):
        with self.stub_runner(error, stop_on) as runner:
            if self.sm_exists:
                ...
                runner.add(
                    stubber.stub_create_state_machine, self.sm_name, self.sm_def,
                    self.sm_role_arn, self.sm_arn)
            runner.add(
                stubber.stub_describe_state_machine, self.sm_arn, self.sm_name, self.sm_def, 'ACTIVE', self.sm_role_arn)
...
def test_find_or_create_state_machine_error(
