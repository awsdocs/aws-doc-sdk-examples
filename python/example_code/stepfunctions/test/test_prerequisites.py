    ...
    def setup_stubs(self, error, stop_on, stubber):
        with self.stub_runner(error, stop_on) as runner:
            if self.role_exists:
                ...
                runner.add(stubber.stub_get_role, self.role_name, error_code='NoSuchEntity')
                runner.add(stubber.stub_create_role, self.role_name, role_arn=self.role_arn)
...
def test_prerequisites(mock_mgr, capsys, role_exists):
    ...
    assert mock_mgr.scenario_data.scenario.state_machine_role['RoleName'] == mock_mgr.role_name
    assert mock_mgr.scenario_data.scenario.state_machine_role['Arn'] == mock_mgr.role_arn
