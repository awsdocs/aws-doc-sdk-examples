...
def test_find_tag_in_projects(
    ...
    with stub_runner(error_code, stop_on_method) as runner:
        ...
        runner.add(
            lookoutvision_stubber.stub_list_tags_for_resource, model_arn, {
                key: value})

    if error_code is None:
        got_project = find_tag.find_tag_in_projects(
            lookoutvision_client, key, value)
        assert got_project == [
            {"Project": project_name, "ModelVersion": model_version}]
    else:
