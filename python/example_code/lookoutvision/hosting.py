...
class Hosting:
    """
    Class showwing how to start and stop an Amazon Lookout for Vision Model. Also shows
    ...
    def start_model(lookoutvision_client, project_name, model_version, min_inference_units):
        ...
        try:
            ...
            if status != "HOSTED":
                raise Exception ("Hosting failed.")
    ...
    def stop_model(lookoutvision_client, project_name, model_version):
        ...
        try:
            ...
            while finished is False:
                ...
                logger.info("Failed to host model: %s ", status)
                finished=True
            ...
            for project in response["Projects"]:
                ...
                for model in response_models["Models"]:
                    ...
                    if model_description["ModelDescription"]["Status"] == "HOSTED":
                        ...
                        hosted += 1
            print (format(hosted) + " model(s) hosted")
        except ClientError as err:
