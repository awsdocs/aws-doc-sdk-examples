...
"""
Uses a trained Amazon Lookour for Vision model to detect anomalies
in an image. The image can be localor in an S3 bucket.
"""
    ...
    def detect_anomalies(lookoutvision_client, project_name, model_version, photo):
        ...
        try:
            ...
            if image_type == "jpeg":
                ...
                raise ValueError(
                    "Invalid file format. Supply a jpeg or png format file: {}".format(photo)
                )
    ...
    def download_from_s3(photo):
        ...
        try:
            ...
            err.response["Error"]["Message"] = (
                "Couldn't download " + photo + " from S3."
            )
            raise
...
def main():
    ...
    try:
        ...
        if args.image.startswith("s3://"):
            photo = Inference.download_from_s3(args.image)
        ...
        print(
            "Your model is {:.0%} confident that the image is {}.".format(
                confidence, state
            )
        )
