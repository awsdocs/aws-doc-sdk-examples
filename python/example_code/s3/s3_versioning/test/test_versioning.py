...
def test_create_versioned_bucket(
    ...
    with stub_runner(error_code, stop_on_method) as runner:
        runner.add(
            ...
            bucket_name,
            versioning.s3.meta.client.meta.region_name,
            keep_going=keep_going,
        )
        runner.add(
            s3_stubber.stub_put_bucket_versioning,
            bucket_name,
            "Enabled",
            keep_going=keep_going,
        )
...
def test_revive_object(
    ...
    obj_key = make_unique_name("object")
    delete_markers = (
        [s3_stubber.make_version(obj_key, "version1", marker_is_latest, datetime.now())]
        if has_delete_markers
        else None
    )
    versions = (
...
def test_permanently_delete_object(make_stubber, make_unique_name, error_code):
    ...
    s3_stubber.stub_delete_object_versions(
        bucket_name,
        [s3_stubber.make_version(obj_key, "version1")],
