...
def test_revise_stanza(
    make_stubber, make_unique_name, make_event, make_result, revision
):
    ...
    s3_stubber.stub_get_object(
        bucket_name, obj_key, object_data=bytes(obj_data, "utf-8")
    )
    if revision == "delete":
        s3_stubber.stub_delete_object(bucket_name, obj_key)
    else:
        s3_stubber.stub_put_object(
            bucket_name, obj_key, body=bytes(revised_data, "utf-8")
        )
