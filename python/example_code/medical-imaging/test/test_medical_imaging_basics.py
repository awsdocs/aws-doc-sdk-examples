...
def test_start_dicom_import_job(make_stubber, error_code):
    ...
    medical_imaging_stubber.stub_start_dicom_import_job(
        job_name, datastore_id, role_arn, input_s3_uri, output_s3_uri, job_id,
        error_code=error_code)

    if error_code is None:
        result = wrapper.start_dicom_import_job(job_name, datastore_id,
                                                role_arn,
                                                input_s3_uri,
                                                output_s3_uri)
        ...
        with pytest.raises(ClientError) as exc_info:
            wrapper.start_dicom_import_job(job_name, datastore_id,
                                           role_arn,
                                           input_s3_uri,
                                           output_s3_uri)
        assert exc_info.value.response['Error']['Code'] == error_code
...
def test_search_mage_sets(make_stubber, error_code):
    ...
    search_filter = {
        "filters": [{
            "values": [{"createdAt": '2023-09-13T14:13:39.302000-04:00'},
...
def test_copy_image_set_with_destination(make_stubber, error_code):
    ...
    medical_imaging_stubber.stub_copy_image_set_with_destination(
        datastore_id, image_set_id, version_id, destination_image_set_id, destination_version_id, error_code=error_code)
