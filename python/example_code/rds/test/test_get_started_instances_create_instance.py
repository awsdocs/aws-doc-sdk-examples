...
class MockManager:
    def __init__(self, stub_runner, instance_data, input_mocker):
        ...
        self.db_engine_versions = [
            {"Engine": self.db_engine, "EngineVersion": f"test-version-{i}"}
            for i in range(1, 4)
        ]
        self.group = {
            "DBParameterGroupFamily": "test-family",
            "DBParameterGroupName": "test-group",
        }
    ...
    def setup_stubs(self, error, stop_on, stubber):
        with self.stub_runner(error, stop_on) as runner:
            ...
            runner.add(
                stubber.stub_describe_db_engine_versions,
                self.db_engine,
                self.db_engine_versions,
                param_family=self.group["DBParameterGroupFamily"],
            )
            runner.add(
                stubber.stub_describe_orderable_db_instance_options,
                self.db_engine,
                self.db_engine_versions[self.engine_choice - 1]["EngineVersion"],
                self.instance_opts,
            )
            runner.add(
                stubber.stub_create_db_instance,
                self.instance_id,
                self.db_engine,
                self.instance_opts[self.instance_choice - 1]["DBInstanceClass"],
                db_name=self.db_name,
                param_group_name=self.group["DBParameterGroupName"],
                db_engine_version=self.db_engine_versions[self.engine_choice - 1][
                    "EngineVersion"
                ],
...
def test_create_instance_exist(mock_mgr, capsys):
    ...
    assert (
        f"'DBInstanceIdentifier': '{mock_mgr.scenario_out['DBInstanceIdentifier']}'"
        in capt.out
    )
