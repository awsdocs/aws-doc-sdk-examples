...
class MockManager:
    ...
    def setup_stubs(self, error, stop_on, stubber):
        with self.stub_runner(error, stop_on) as runner:
            ...
            runner.add(
                stubber.stub_create_db_parameter_group,
                self.group_name,
                "test-group-1",
...
def test_create_parameter_group_exist(mock_mgr, capsys):
    mock_mgr.instance_data.stubber.stub_describe_db_parameter_groups(
        mock_mgr.group_name
    )

    got_output = mock_mgr.instance_data.scenario.create_parameter_group(
    ...
    assert (
        f"'DBParameterGroupName': '{mock_mgr.scenario_out['DBParameterGroupName']}'"
        in capt.out
    )
