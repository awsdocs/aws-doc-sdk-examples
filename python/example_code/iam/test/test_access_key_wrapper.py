...
def test_delete_key(make_stubber, make_unique_name, error_code):
    ...
    iam_stubber.stub_delete_access_key(
        user_name, key_id, error_code=error_code)
...
def test_get_last_use(make_stubber, make_unique_name, error_code):
    ...
    iam_stubber.stub_get_access_key_last_used(
        key_id, user_name, error_code=error_code)
...
def test_list_keys(make_stubber, make_unique_name, error_code):
    ...
    user_name = make_unique_name("user-")
    key_ids = [
        f"test-key-id-plus-extra-words-{index}" for index in range(1, 5)]

    iam_stubber.stub_list_access_keys(
        user_name, key_ids, error_code=error_code)
...
@pytest.mark.parametrize("activate,error_code",
                         [(True, None), (False, None), (True, "NoSuchEntity")])
