...
def test_query_test(make_stubber):
    ...
    for _ in range(iterations):
        dyn_stubber.stub_query(
            TRY_DAX_TABLE,
            [
                {"partition_key": partition, "sort_key": sort, "some_data": "X" * 100}
                for sort in range(sorts[0], sorts[1])
            ],
            key_condition=Key("partition_key").eq(partition)
...
def test_scan_test(make_stubber):
    ...
    for _ in range(iterations):
        dyn_stubber.stub_scan(
            TRY_DAX_TABLE,
            [
                {"partition_key": key, "sort_key": key, "some_data": "X" * 100}
