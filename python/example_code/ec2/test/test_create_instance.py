...
class MockManager:
    def __init__(self, stub_runner, scenario_data, input_mocker):
        ...
        self.images = [
            MagicMock(
                id=f"test-id-{ind}", description=f"desc-{ind}", architecture="test-arch"
            )
        ...
        self.instance = MagicMock(
            ...
            instance_type=self.inst_types[0],
    ...
    def setup_stubs(self, error, stop_on, stubber):
        with self.stub_runner(error, stop_on) as runner:
            runner.add(
                self.scenario_data.ssm_stubber.stub_get_parameters_by_path,
                self.image_names,
            ...
            runner.add(
                stubber.stub_create_instances,
                self.images[0].id,
                self.inst_types[0],
                self.scenario_data.scenario.key_wrapper.key_pair.name,
                1,
                self.instance.id,
