...
def test_poll_notification(make_stubber, stub_runner, error_code, stop_on_method):
    ...
    with stub_runner(error_code, stop_on_method) as runner:
        ...
        runner.add(
            ...
            queue_url,
            MagicMock(receipt_handle="Receipt-0"),
        )
...
@pytest.mark.parametrize(
    "poll_status,error_code,stop_on_method",
    [
        ("SUCCEEDED", None, None),
        ("FAILED", None, None),
...
def test_do_label_detection(
    ...
    with stub_runner(error_code, stop_on_method) as runner:
        runner.add(
            rekognition_stubber.stub_start_detection,
...
def test_do_face_detection(
    ...
    if error_code is None:
        ...
        assert [face.to_dict() for face in faces] == [
...
def test_do_person_tracking(
    ...
    if error_code is None:
        ...
        assert [person.to_dict() for person in persons] == [
...
def test_do_celebrity_recognition(
    ...
    video = mock_video(monkeypatch, "SUCCEEDED", rekognition_client)
    celebrities = [
        RekognitionCelebrity(celebrity, time.time_ns())
    ...
    with stub_runner(error_code, stop_on_method) as runner:
        ...
        runner.add(
            rekognition_stubber.stub_get_celebrity_recognition,
            job_id,
            job_status,
            celebrities,
        )
    ...
    if error_code is None:
        ...
        assert [celebrity.to_dict() for celebrity in celebrities] == [
