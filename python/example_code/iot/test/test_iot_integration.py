...
@pytest.mark.integ
def test_scenario_complete_integration(capsys):
    """Test the complete IoT scenario flow against real AWS services."""
    IoTScenario.is_interactive = False
    ...
    rule_name = f"test_rule_{int(__import__('time').time())}"

    with patch("demo_tools.question.ask", return_value="y"):
        with patch.object(scenario, "_deploy_stack", return_value=("arn:aws:sns:us-east-1:123456789012:test", "arn:aws:iam::123456789012:role/test")):
            with patch.object(scenario, "_cleanup_stack"):
                with patch.object(iot_wrapper, "create_topic_rule"):
                    with patch.object(iot_wrapper, "delete_topic_rule"):
...
def test_create_thing_integration():
    ...
    iot_client = boto3.client("iot")
    iot_wrapper = IoTWrapper(iot_client)
    thing_name = f"test-thing-{int(__import__('time').time())}"
