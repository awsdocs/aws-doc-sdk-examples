...
class MockManager:
    def __init__(self, stub_runner, scenario_data, input_mocker):
        ...
        self.template = s3_express_getting_started.S3ExpressScenario.get_template_as_string()
    ...
    def setup_stubs(self, error, stop_on, ec2_stubber, cloud_formation_stubber, monkeypatch):
        with self.stub_runner(error, stop_on) as runner:
            runner.add(ec2_stubber.stub_create_vpc, "10.0.0.0/16", self.vpc_id)
            runner.add(
                ec2_stubber.stub_describe_route_tables, self.filters, self.vpc_id, self.route_table_id
            )
            runner.add(
                ec2_stubber.stub_create_vpc_endpoint, self.vpc_id, self.route_table_id, self.service_name
            )
            runner.add(
        ...
        # Mock the waiters.
        monkeypatch.setattr(waiter.Waiter, "wait", lambda self, **kwargs : None)
