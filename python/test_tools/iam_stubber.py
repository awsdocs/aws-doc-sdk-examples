...
class IamStubber(ExampleStubber):
    ...
    def stub_get_policy(
        ...
        response = {
            ...
            "Policy": {"PolicyName": policy_arn.split(":")[-1], "Arn": policy_arn},
        }
    ...
    def stub_list_policies(self, scope, policies, error_code=None):
        ...
        response = {
            "Policies": [
                {"PolicyName": poli_key, "Arn": poli_val}
    ...
    def stub_list_attached_role_policies(
        ...
        response = {
            "AttachedPolicies": [
                {"PolicyName": name, "PolicyArn": arn} for name, arn in policies.items()
            ]
        }
        self._stub_bifurcator(
    ...
    def stub_create_instance_profile(
        self, profile_name, profile_arn=None, error_code=None
    ):
    ...
    def stub_delete_service_linked_role(
        self, role_name, deletion_task_id, error_code=None
    ):
        expected_params = {"RoleName": role_name}
    ...
    def stub_get_service_linked_role_deletion_status(
        self, task_id, status, error_code=None
    ):
