...
class ELBv2Stubber(ExampleStubber):
    ...
    def stub_create_target_group(self, tg_name, protocol, port, vpc_id, healthcheck, tg_arn, error_code=None):
        expected_params = {
            'Name': tg_name, 'Protocol': protocol, 'Port': port, 'VpcId': vpc_id,
            'HealthCheckPath': healthcheck['path'], 'HealthCheckIntervalSeconds': healthcheck['interval'],
            'HealthCheckTimeoutSeconds': healthcheck['timeout'], 'HealthyThresholdCount': healthcheck['thresh_healthy'],
            'UnhealthyThresholdCount': healthcheck['thresh_unhealthy']}
        response = {'TargetGroups': [{
            'TargetGroupName': tg_name, 'TargetGroupArn': tg_arn, 'Protocol': protocol, 'Port': port}]}
    ...
    def stub_describe_target_health(self, tg_arn, tg_descs, error_code=None):
        ...
        response = {
            'TargetHealthDescriptions': [
                {'TargetHealth': {'State': desc['state'], 'Reason': desc['reason'], 'Description': desc['desc']},
