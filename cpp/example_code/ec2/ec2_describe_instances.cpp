...
int main(int argc, char** argv)
{
    ...
    while(!done)
    {
        ...
        if(describeInstancesOutcome.IsSuccess())
        {
            ...
            for(const auto& reservation : reservations)
            {
                ...
                for (const auto& instance : instances)
                {
                    ...
                    Aws::String typeString = Aws::EC2::Model::InstanceTypeMapper::GetNameForInstanceType(instance.GetInstanceType());
                    Aws::String monitoringString = Aws::EC2::Model::MonitoringStateMapper::GetNameForMonitoringState(instance.GetMonitoring().GetState());
                    Aws::String nameString = "Unknown";
                
                    const auto& tags = instance.GetTags();
                    auto nameIter = std::find_if(tags.cbegin(), tags.cend(), [](const Aws::EC2::Model::Tag& tag){ return tag.GetKey() == "Name"; });
        ...
        {
            std::cout << "Failed to describe ec2 instances:" << describeInstancesOutcome.GetError().GetMessage() << std::endl;
            done = true;
        }
    }
