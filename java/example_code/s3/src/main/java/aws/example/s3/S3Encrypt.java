...
/**
 * This AWS Code Example is Depreciated. To encrypt S3 content, see the
 * S3EncryptV2 example.
 */
public class S3Encrypt {
    ...
    /**
     * Uses AES/GCM with AESWrap key wrapping to encrypt the key. Uses v2 metadata
     * schema. Note that authenticated
     * encryption requires the bouncy castle provider to be on the classpath. Also,
     * for authenticated encryption the size
    ...
    /**
     * For ranged GET we do not use authenticated encryption since we aren't reading
     * the entire message and can't produce the
     * MAC. Instead we use AES/CTR, an unauthenticated encryption algorithm. If
     * {@link CryptoMode#StrictAuthenticatedEncryption}
     * is enabled, ranged GETs will not be allowed since they do not use
    ...
    /**
     * Same as {@link #authenticatedEncryption_CustomerManagedKey()} except uses an
    ...
    /**
     * Uses AES/GCM with AESWrap key wrapping to encrypt the key. Uses v2 metadata
     * schema. The only difference between this and
     * {@link #authenticatedEncryption_CustomerManagedKey()} is that attempting to
     * retrieve an object non
     * encrypted with AES/GCM will thrown an exception instead of falling back to
     * encryption only or plaintext GET.
     */
    ...
    /**
     * Strict authenticated encryption mode does not support ranged GETs. This is
     * because we must use AES/CTR for ranged
     * GETs which is not an authenticated encryption algorithm. To do a partial get
     * using authenticated encryption you have to
    ...
    /**
     * Uses an asymmetric key pair instead of a symmetric key. Note this does not
     * change the algorithm used to encrypt
     * the content, that will still be a symmetric key algorithm (AES/CBC in this
     * case) using the derived CEK. It does impact
     * the algorithm used to encrypt the CEK, in this case we use
    ...
    /**
     * This uses the V2 metadata schema with a key wrap algorithm of 'kms' and a CEK
     * algorithm of AES/CBC/PKCS5Padding.
     */
    ...
    /**
     * This uses the V2 metadata schema with a key wrap algorithm of 'kms' and a CEK
     * algorithm of AES/GCM/NoPadding.
     */
    ...
    /**
     * Same as authenticatedEncryption_KmsManagedKey except throws an exception when
     * trying to get objects not encrypted with
     * AES/GCM.
     */
