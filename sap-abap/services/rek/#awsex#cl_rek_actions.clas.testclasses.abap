...
CLASS ltc_awsex_cl_rek_actions IMPLEMENTATION.
  ...
  METHOD class_setup.
    ...
    " Tag the bucket
    TRY.
        DATA(lt_tagset) = VALUE /aws1/cl_s3_tag=>tt_tagset(
          ( NEW /aws1/cl_s3_tag( iv_key = 'convert_test' iv_value = 'true' ) ) ).
        DATA(lo_tagging) = NEW /aws1/cl_s3_tagging( it_tagset = lt_tagset ).
    ...
    create_test_collection( iv_collection_id = av_collection_id ).
    wait_for_collection_ready( iv_collection_id = av_collection_id ).

    " Create collection for delete test
    create_test_collection( iv_collection_id = av_collection_id_for_delete ).
    wait_for_collection_ready( iv_collection_id = av_collection_id_for_delete ).

    " Upload test images to S3 for use in tests
    upload_test_image( iv_key = av_test_image_key ).
    upload_test_image( iv_key = av_test_image_key2 ).
  ...
  METHOD class_teardown.
    ...
    TRY.
        delete_test_image( iv_key = av_test_image_key ).
      CATCH /aws1/cx_rt_generic.
    ENDTRY.

    TRY.
        delete_test_image( iv_key = av_test_image_key2 ).
      CATCH /aws1/cx_rt_generic.
    ENDTRY.
    ...
    TRY.
        delete_test_collection( iv_collection_id = av_collection_id ).
      CATCH /aws1/cx_rt_generic.
    ENDTRY.

    TRY.
        delete_test_collection( iv_collection_id = av_collection_id_for_delete ).
      CATCH /aws1/cx_rt_generic.
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD create_test_collection.
    TRY.
        ...
        DATA(lt_tags) = VALUE /aws1/cl_rektagmap_w=>tt_tagmap(
          ( key = 'convert_test' value = 'true' ) ).
  ...
  METHOD upload_test_image.
    ...
    lv_jpeg_data = ''.
  ...
  METHOD wait_for_collection_ready.
    ...
    WHILE lv_attempt < lv_max_attempts.
      TRY.
          DATA(lo_result) = ao_rek->describecollection(
            iv_collectionid = iv_collection_id ).
  ...
  METHOD create_collection.
    ...
    DATA(lv_uuid) = /awsex/cl_utils=>get_random_string( ).
    TRANSLATE lv_uuid TO LOWER CASE.
    DATA(lv_test_collection_id) = |sap-abap-rek-create-{ lv_uuid }|.
    ...
    TRY.
        ...
        DATA(lv_status_code) = lo_result->get_statuscode( ).
  ...
  METHOD describe_collection.
    ...
    TRY.
        ...
        DATA(lv_collection_arn) = lo_result->get_collectionarn( ).
  ...
  METHOD list_collections.
    ...
    TRY.
        ...
        DATA(lt_collections) = lo_result->get_collectionids( ).
        cl_abap_unit_assert=>assert_not_initial(
          act = lt_collections
        ...
        LOOP AT lt_collections INTO DATA(lv_collection_id).
  ...
  METHOD index_faces.
    ...
    TRY.
        ...
        DATA(lt_face_records) = lo_result->get_facerecords( ).
        IF lt_face_records IS NOT INITIAL.
          DATA(lo_face) = lt_face_records[ 1 ]->get_face( ).
          IF lo_face IS BOUND.
  ...
  METHOD list_faces.
    ...
    IF av_indexed_face_id IS INITIAL.
      " Index a face if not already done
        TRY.
            DATA(lo_index_result) = ao_rek->indexfaces(
    ...
    TRY.
        ...
        DATA(lt_faces) = lo_result->get_faces( ).
        cl_abap_unit_assert=>assert_not_initial(
          act = lt_faces
  ...
  METHOD search_faces.
    ...
    IF av_indexed_face_id IS INITIAL.
      TRY.
          ...
          DATA(lt_face_records) = lo_index_result->get_facerecords( ).
          IF lt_face_records IS NOT INITIAL.
            DATA(lo_face) = lt_face_records[ 1 ]->get_face( ).
            IF lo_face IS BOUND.
              av_indexed_face_id = lo_face->get_faceid( ).
  ...
  METHOD delete_faces.
    ...
    TRY.
        ...
        DATA(lt_deleted_faces) = lo_result->get_deletedfaces( ).
        cl_abap_unit_assert=>assert_not_initial(
