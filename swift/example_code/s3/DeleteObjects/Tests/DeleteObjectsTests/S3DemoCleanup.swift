...
public class S3DemoCleanup {
    ...
    /// Reset the demo cleanup handler.
    /// - Parameters:
    ///   - dropBuckets: If `true`, existing buckets are dropped without being
    ///     deleted.
    ///   - dropFiles: If `true`, existing files are dropped without being
    ///     deleted.
    func reset(dropBuckets: Bool = true, dropFiles: Bool = true) {
        if dropBuckets {
            self.buckets = []
        }
        if dropFiles {
