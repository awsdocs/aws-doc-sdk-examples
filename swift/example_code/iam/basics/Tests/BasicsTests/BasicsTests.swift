...
final class BasicsTests: XCTestCase {
    ...
    func testBasics() async throws {
        ...
        do {
            let createdID = try await BasicsTests.serviceHandler!.basics(name: name)
            let getID = try await BasicsTests.serviceHandler!.getUserID(name: name)

            XCTAssertTrue(createdID == getID, "Created user's ID (\(createdID)) doesn't match retrieved user's ID (\(getID))")

            // Delete the created user.
            _ = try await BasicsTests.serviceHandler!.deleteUser(name: name)
        } catch {
            throw error
        }
    }
