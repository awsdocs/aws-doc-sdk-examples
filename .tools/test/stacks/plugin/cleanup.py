...
def terminate_runnable_jobs_in_queue(job_queue):
    ...
    runnable_jobs = batch_client.list_jobs(jobQueue=job_queue, jobStatus="RUNNABLE")
    ...
    for job in runnable_jobs["jobSummaryList"]:
        ...
        try:
            response = batch_client.terminate_job(
                jobId=job_id,
                reason="Terminating all RUNNABLE jobs in stack weathertop plugin",
            )
            print(
                f"Terminated job ID: {job_id} in queue {job_queue} in in stack weathertop plugin"
            )
        except Exception as e:
            print(
                f"Failed to terminate job ID: {job_id} in queue {job_queue}. Error: {e}"
            )
...
def discover_and_delete_runnable_jobs():
    ...
    for queue in job_queues:
        if queue["state"] == "ENABLED":  # Filter for enabled queues if necessary
            ...
            print(f"Processing weathertop plugin stack queue: {job_queue_name}")
