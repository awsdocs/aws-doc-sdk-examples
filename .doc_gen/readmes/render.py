...
class Renderer:
    ...
    def _transform_sdk(self):
        ...
        if 'expanded' not in pre_sdk or 'guide' not in pre_sdk:
            logger.error(
                ...
                self.scanner.lang_name, self.sdk_ver)
            raise MissingMetadataError
        else:
            post_sdk = {
    ...
    def _transform_service(self):
        ...
        if 'expanded' not in pre_svc or 'blurb' not in pre_svc or 'guide' not in pre_svc:
            logger.error(
                "%s entry in services.yaml does not have required entity expansions, blurb, or guide "
                "info defined.", self.scanner.svc_name)
            raise MissingMetadataError
        else:
            post_svc = {
    ...
    def _transform_actions(self, pre_actions):
        ...
        for _, pre in pre_actions.items():
            # print(f"svc name "+self.scanner.svc_name + pre['services'])
    ...
    def _transform_scenarios(self):
        ...
        for pre_id, pre in pre_scenarios.items():
            scenario = {
                ...
                'file': self.scanner.snippet(pre, self.sdk_ver, self.lang_config['service_folder'], ''),
            }
            if scenario['file'] is None:
                logger.warning("Couldn't find file for scenario: %s.", scenario['title_abbrev'])
            else:
        ...
        for _, pre in pre_crosses.items():
            ...
            if github is None:
                ...
                if base_folder in github:
                    github = self._lang_level_double_dots() + github.split(base_folder, 1)[1]
                cross = {
                    'title_abbrev': pre['title_abbrev'],
                    'file': github,
                }
