...
This document contains the technical specifications for Feedback Sentiment Analyzer (FSA), a sample application that showcases Amazon Textract, Amazon Comprehend, Amazon Translate, and Amazon Polly. The preceding services are called using the [supported AWS SDKs](https://aws.amazon.com/developer/tools/).

## FSA overview

FSA was created for the fictional manger of a hotel in France. You can read more about its origins in [DESIGN.md](./DESIGN.md). The deployed application provides a frontend client that connects to several AWS services via Amazon API Gateway and Amazon CloudFront. End-users can use the client upload images containing text in any supported language (English, French, German, Italian, Portuguese, and Spanish). The text from the image is translated to French and synthesized into audio. The client displays the original uploaded image, the translated text, and the audio.
...
1. Authentication and environment Lambda (API Gateway, Cognito, Lambda)
2. Uploading an image (API Gateway, S3, EventBridge)
3. Analyzing sentiment, translating, and synthesizing audio (Step Functions, Textract, Comprehend, Translate, Polly, S3, DynamoDB, Lambda)
4. Displaying results in the client (API Gateway, Lambda, DynamoDB, S3)
...
| - | - | - | - |
| `/feedback` | GET | [GetFeedback Lambda function](#GetFeedback-lambda-function) | `[   {     "text": String,     "audioUrl": String   } ]` |
